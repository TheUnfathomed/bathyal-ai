"""Discover 200+ well-represented biological taxa from FathomNet for expanded training."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Discover expanded FathomNet taxa for training.")
    parser.add_argument("--min-count", type=int, default=30, help="Minimum bounding box count per concept.")
    parser.add_argument("--top-n", type=int, default=200, help="Number of top concepts to output.")
    parser.add_argument("--output", type=str, default="scripts/concepts_200.txt", help="Output concepts file.")
    parser.add_argument("--output-json", type=str, default="scripts/concepts_200_metadata.json", help="Output metadata JSON.")
    ...
    metadata = [
        {"concept": entry.concept, "fathomnet_box_count": entry.count, "rank": i}
        for i, entry in enumerate(selected, 1)
    ]
